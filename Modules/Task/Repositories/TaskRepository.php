...
class TaskRepository
{
    ...
    public function create($request)
    {
        ...
        if (isset($input['estimated_hours'])) {
            if (isset($input['parent_task_id']) && $input['parent_task_id']) {
                ...
                $parentTask->estimated_hours = $this->getTasksEstimatehours([$parentTask->estimated_hours, $input['estimated_hours']], true);
                $parentTask->save();
            }
        }
    ...
    public function update($request, $id)
    {
        ...
        if (isset($input['parent_task_id']) && $input['parent_task_id']) {
            ...
            if($task->estimated_hours) {
                $parentTask->estimated_hours = $this->getTasksEstimatehours([$parentTask->estimated_hours, $task->estimated_hours]);
                $parentTask->save();
            }
    ...
    private function _validateTask($data, $date, $status, $priority, $project)
    {
        ...
        if (!empty($data[5])) {
            ...
            } elseif ( $date['plannedStartDate'] != null || $date['plannedEndDate'] != null ) {
                if ((!($date['taskStartDate'] >= $date['plannedStartDate'] && $date['taskStartDate'] <= $date['plannedEndDate']))) {
        ...
        if (!empty($data[6])) {
            ...
            } elseif ($date['plannedEndDate'] != null && !($date['taskEndDate'] <= $date['plannedEndDate'])) {
                $error[] = 'The task end date must be a date before or equal to planned end date.';
            }
        }
     ...
     * @return String
     */
    private function getTasksEstimatehours($times = [], $isCreated = false) {
        $sumSeconds = 0;
        if($isCreated) {
