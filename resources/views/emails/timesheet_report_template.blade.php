                    ...
                    <tr bgcolor="#D3D3D3">
                        <td colspan="5">
                            <div style="text-align: center;">
                                <strong><a href="{{$front_url}}#/users/profile/{{$timesheet->user->id}}">{{$timesheet->user->firstname}} {{$timesheet->user->lastname}}</a> - Total :: {{$user_hours[$timesheet->created_user_id]}}</strong>
                            </div>
                        </td>
                    </tr>
                    @endif
                    <tr>
                        <td>
                            @if ($timesheet->module_id == 2)
                                <a href="{{$front_url}}#/tasks/detail/{{$timesheet->module_related_id}}">#{{$timesheet->task_generated_id}}</a>
                                 -  {{$timesheet->task_name}}
                            @elseif ($timesheet->module_id == 3)
                                <a href="{{$front_url}}#/defects/detail/{{$timesheet->module_related_id}}">#{{$timesheet->defect_generated_id}}</a>
                                -  {{$timesheet->defect_name}}
                            @elseif ($timesheet->module_id == 4)
                                <a href="{{$front_url}}#/incidents/detail/{{$timesheet->module_related_id}}">#{{$timesheet->incident_generated_id}}</a>
                                 - {{$timesheet->incident_name}}
                            @elseif ($timesheet->module_id == 5)
                                <a href="{{$front_url}}#/meetings/detail/{{$timesheet->module_related_id}}">#{{$timesheet->module_related_id}}</a>
                                 - {{$timesheet->meeting_title}}
